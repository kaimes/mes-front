<template>
  <v-container>
    <v-row no-gutter>
      ...
      <v-spacer />
      <v-tooltip bottom text="Add Plugin">
        <template v-slot:activator="{ props }">
          <v-btn small icon @click="addPlugin()" v-bind="props"
    ...
    <span v-for="(plugin, idx) in plugins" :key="plugin.slug">
      <v-row align="center" dense>
        <v-col cols="12" sm="1">
          <v-tooltip bottom text="Remove Plugin">
            <template v-slot:activator="{ props }">
              <v-btn small icon @click="removePlugin(idx)" v-bind="props"
        ...
        <v-col cols="12" sm="1">
          <v-tooltip bottom text="Add Item">
            <template v-slot:activator="{ props }">
              <v-btn small icon @click="addItem(plugin)" v-bind="props"
                ><v-icon>add</v-icon></v-btn
              >
            </template>
            <span></span>
      ...
      <v-row
        align="center"
        dense
        v-for="meta in plugin.metadata"
        :key="meta.key"
      >
        <v-col cols="12" sm="1">
          <v-tooltip bottom text="Remove Item">
            <template v-slot:activator="{ props }">
              <v-btn small icon @click="removeItem(plugin)" v-bind="props"
                ><v-icon>remove</v-icon></v-btn
              >
            </template>
          </v-tooltip>
