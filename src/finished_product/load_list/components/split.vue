...
const allOptions = [60, 80, 120, 180]; // Define all possible split options

const filteredOptions = computed(() => {
  if (!selectedQualifiedQuantity.value) return allOptions;
  return allOptions.filter(option => option <= selectedQualifiedQuantity.value);
});
...
const handleShow = (data) => {
  const totalBars = data[0]?.qualified_quantity || 0;

  // Check if total bars are less than 60
  if (totalBars < 60) {
    toast.error("Splitting is not allowed for bundles with less than 60 bars.", { autoClose: 60000 });
    return; // Prevent the dialog from opening
  }
