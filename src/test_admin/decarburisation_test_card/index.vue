    ...
    <v-form ref="searchForm" @submit.prevent="handleSearch" @keydown.enter="handleSearch">
      ...
      <div class="panel border">
        <v-row align="center">
          ...
          <v-col cols="3">
            <v-text-field v-model="form.max" color="#333" label="MAX(mm)" :disabled="!save" density="default"
      ...
      <v-card title="Test Requirement" class="panel-card">
        <div class="panel border">
          <v-row align="center">
            <v-col cols="4">
              <v-text-field v-model="form.decarburisation_min" color="#333" label="Derification MIN(mm)" :disabled="!save"
                density="default" variant="underlined" placeholder="Derification MIN(mm)" bg-color="white" clearable>
              </v-text-field>
            </v-col>
            <v-col cols="4">
              <v-text-field v-model="form.decarburisation_max" color="#333" label="Derification MAX(mm)"
                :disabled="!save" density="default" variant="underlined" placeholder="Derification MAX(mm)"
                bg-color="white" clearable>
              </v-text-field>
            </v-col>
          </v-row>
      ...
      <v-card title="Result" class="panel-card">
        <div class="panel border">

          <v-row align="center">
            <v-col cols="4">
              <v-text-field v-model="form.decarburisation" color="#333" label="Derification (mm)"
                :disabled="!save" density="default" variant="underlined" placeholder="Derification (mm)"
                bg-color="white" clearable>
              </v-text-field>
            </v-col>
          </v-row>
...
const data = reactive({
  ...
  form: {
    ...
    decarburisation_min: undefined,
    decarburisation_max: undefined,

    decarburisation: undefined,
...
const getBendTestCardId = (id) => {
  API.getDetailById(id).then(res => {
    ...
    if (status === 200) {
      save.value = false;
      form.value = data;
      // form.value.test_code = data.test_code
      form.value.section = data.section
      form.value.section_size_code = data.section_size_code
      form.value.test_ref = data.test_ref
      form.value.bloom = data.bloom
      form.value.cast_number = data.cast.cast_code
      // form.value.inspection_authority_code = data.spec.inspector_code
      form.value.inspection_authority_code = data.inspection_authority_code
      form.value.kg_m = data.kg
      form.value.spec_code = data.spec.spec_code
      form.value.max = data.max
      form.value.status = data.status
      form.value.decarburisation = data.decarburisation
      form.value.decarburisation_min = data.decarburisation_min
      form.value.decarburisation_max = data.decarburisation_max
