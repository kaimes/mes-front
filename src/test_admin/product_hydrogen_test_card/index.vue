    ...
    <v-form ref="searchForm" @submit.prevent="handleSearch" @keydown.enter="handleSearch">
      ...
      <div class="panel border">
        <v-row align="center">
          ...
          <v-col cols="3">
            <v-text-field v-model="form.max" color="#333" label="MAX(ppm)" :disabled="!save" density="default"
      ...
      <v-card title="" class>
        <div class="panel border">
          <v-row align="center">
            <v-col cols="4">
              <v-text-field v-model="form.spec_test" color="#333" label="Spec" :disabled="!save"
                density="default" variant="underlined" placeholder="Spec" bg-color="white" clearable>
              </v-text-field>
            </v-col>
            <v-col cols="4">
              <v-text-field v-model="form.rail_grade" color="#333" label="Rail Grade"
                :disabled="!save" density="default" variant="underlined" placeholder="Rail Grade"
                bg-color="white" clearable>
              </v-text-field>
            </v-col>
            <v-col cols="4">
              <v-text-field v-model="form.max_test" color="#333" label="MAX(ppm)"
                :disabled="!save" density="default" variant="underlined" placeholder="MAX(ppm)"
                bg-color="white" clearable>
              </v-text-field>
            </v-col>
          </v-row>
      ...
      <v-card title="" class>
        <div class="panel border">
          ...
          <v-row align="center">
            <v-col cols="4">
              <v-text-field v-model="form.result" color="#333" label="Result (ppm)"
                :disabled="!save" density="default" variant="underlined" placeholder="Result (ppm)"
                bg-color="white" clearable>
              </v-text-field>
            </v-col>
          </v-row>
...
const getBendTestCardId = (id) => {
  API.getDetailById(id).then(res => {
    ...
    if (status === 200) {
      ...
      form.value.max = data?.spec.max
      form.value.status = data?.status

      form.value.spec_test = data?.spec_test
      form.value.rail_grade = data?.rail_grade
      form.value.max_test = data?.max_test
