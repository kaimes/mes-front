<template>
  ...
  >
    <v-card
      class="ma-0 pa-0"
      max-width="240"
      style="margin-right: 20px !important; height: 68px"
    >
      <v-list-item class="ma-0" density="default">
        ...
        <v-list-item-title class="text-xs-caption">
          ...
          {{ company_show_flag ? 'Company' : 'Worker' }} :{{
            company_show_flag ? 'company-name' : name
          }}
        </v-list-item-title>
    ...
    <v-chip-group v-if="taxi_kpi_show_flag" column>
      <v-tooltip
        bottom
        v-for="(val, key, index) in taxi_kpi"
        :key="index"
        :text="key + `:` + val.value"
      >
    ...
    <v-spacer />
    <v-switch v-model="visibleAutoNavi" class="mx-2" label="Bus"></v-switch>
