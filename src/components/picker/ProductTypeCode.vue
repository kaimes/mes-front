...
<script setup>
import {debounce, unionBy, find, includes, trim, some} from "lodash";
...
const props = defineProps({
  ...
  label: {
    type: String,
    default: 'Section Weight'
  },
  /**输入框内部的文字 */
  placeholder: {
    type: String,
    default: 'Please Select Section Weight'
  },
  /** 多选 */
  multiple: {
    type: Boolean,
  ...
  if (val) {
    ...
    try {
      ...
      if (status === 200) {
        if (props.multiple) {
          // 合并选择的 和 新查出来的
          items.value = unionBy(data.items, selItems.value, 'id')
        } else {
          // 单选
...
const getData = async (ids) => {
  ...
  if (status === 200) {
      if (props.multiple) {
        const its =  (props.product_class || props.product_category) ? data.items :  unionBy(data.items, items.value, 'id')
        $data = unionBy(its, selItems.value, 'id')
      }else{
        $data = data.items || []
      }
    if (props.product_type?.id) {
      $data = [
        ...$data,
        toRaw(props.product_type),
      ];
    }
    items.value = $data;
  } else {
