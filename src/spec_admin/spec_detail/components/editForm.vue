...
const handleSave = async () => {
  ...
  loading.value = true;
  let remarkId = getIdByCode(data.remarkCode)
  console.log(remarkId);
  
  if (!route.query.id || !remarkId){
  ...
  const requestData = {
    ...
    "spec_id": route.query.id
     ...
     API.commonAddRequest("/remark/spec_remark/create",requestData).then(reponse => {
      getAllDate()
      handleClose();
      emit("refresh");
...
const getAllDate = () =>{
  API.commonGetRequest(`/remark/spec_remark/get_remark/${route.query.id}`).then(reponse => {
