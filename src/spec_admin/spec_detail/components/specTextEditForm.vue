<template>
  ...
  >
    <v-form ref="editForm">
      ...
      <v-row align="center">
        <v-col cols="qw">
          <DictCode v-model="form.spec_text_type" code="spec_text_type" label="Spec Text Type" :hide-details="false"/>
        </v-col>
      </v-row>
      <v-row align="center">
        <v-col cols="qw">
...
const handleSave = async () => {
  ...
  if (form.value.id) {
    let payload = {
      ...
      "spec_text_type": form.value.spec_text_type
    }
    API.commonUpdateRequest(`/remark/spec_remark/update_spec_text/${form.value.id}`, payload).then(reponse => {
      handleClose();
      emit("refresh");
  ...
  API.commonAddRequest("/remark/spec_remark/create_spec_text", requestData).then(reponse => {
  handleClose();
  emit("refresh");
