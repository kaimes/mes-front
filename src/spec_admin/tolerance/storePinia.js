...
import ToleranceApi from "@/spec_admin/tolerance/api";
...
const actions = {
  getAll() {
    ...
    return ToleranceApi.getAll(this.table.options)
      .then((response) => {
  ...
  removeShow(tolerance) {
    this.SET_DIALOG_DELETE(true);

    this.SET_SELECTED(tolerance);
  },
  closeCreateEdit() {
  ...
  save() {
    ...
    if (!that.isEditFlag) {
      ...
      return ToleranceApi.create(that.selected)
        .then(() => {
      ...
      return ToleranceApi.update(that.selected)
        .then(() => {
          that.closeCreateEdit();
          that.getAll();
  ...
  remove() {
    const that = this;
    return ToleranceApi.delete(that.selected.id)
      .then(function () {
        that.closeRemove();
        that.getAll();
...
const namespace = 'tolerance'
export const useToleranceStore = defineStore(namespace, {
  id: namespace,
  state: () => {
    return state;
  },
