  ...
  <v-container fluid>
    ...
    <v-form ref="searchForm" @submit.prevent="handleSearch" @keydown.enter="handleSearch">
      <v-row align="center">
        ...
        <!-- <v-col cols="6" class="text-end">
          <router-link :to="{ path: '/tolerance_detail' }">
            <v-btn
                color="primary"
                variant="flat"
                v-permission="`Create`"
      ...
      >
        <template v-slot:[`item.action`]="{ item }">
          <div class="d-flex align-center ga-4">
            <router-link :to="{ path: '/tolerance_detail', query: { id: item.id } }">
...
<script>
import DeleteDialog from "@/spec_admin/tolerance/DeleteDialog.vue";

import NewEditSheet from "@/spec_admin/tolerance/NewEditSheet.vue";
...
export default {
  ...
  data() {
    return {
      headers: [
        ...
        { title: "Mill Code", key: "mill_code", value: (item) => item.mill?.code || "--", sortable: false },
        { title: "Code", value: "code", sortable: true },
        { title: "Type", value: "type", sortable: false },
        { title: "Name", value: "name", sortable: false },
        { title: "Description", value: "desc", sortable: false },
      ],
  ...
  computed: {
    ...
    ...mapState(useToleranceStore, ["table"]),
  },
