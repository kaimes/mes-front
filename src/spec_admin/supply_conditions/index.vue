...
import { te } from "date-fns/locale";
  ...
  const tableHeaders = ref([
    ...
    {title: "Filler", key: "filler"},
    {title: "Code", key: "code"},
    {title: "Text", key: "text", value: (item) => item.text}
  ]);
  const tableList = ref([]);
  const total = ref(0);
  ...
  const data = reactive({
    ...
    spscond_code: commonStore.dict.spscond_code
  });
  
  const {queryParams, form, spscond_code} = toRefs(data);
      ...
      if (res.status === 200) {
        for (let item of res.data.items){
          if (item.code != null){
            ...
            let spscode_value = spscond_code.value.find(spvalue => spvalue.value == item.code);
            
            if (spscode_value){
              item.text = spscode_value.text;
            }
